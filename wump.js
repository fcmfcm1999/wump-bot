...
async function getUserTasks(token, userId, userAgent) {
    try {
...
async function completeReferralTask(token, taskId, userAgent) {
    try {
        const response = await axios.post(
            'https://api.wump.xyz/functions/v1/api/tasks/referral',
            {
                taskid: taskId,
            },
            {
...
async function main() {
    ...
    try {
        ...
        for (const token of tokens) {
            ...
            const completedTasks = await getUserTasks(token, userInfo.id, userAgent)
            ...
            logger.success('Finished processing all tasks for this token\n')
        }
